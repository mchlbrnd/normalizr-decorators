...
import { ArrayProperty, denormalize, Entity, EntityProperty, normalize } from './../src';
...
beforeEach(() => {
  const {result, entities} = expectedNormalizedData = normalizr.normalize(originalData, article);
  expectedDenormalizedData = normalizr.denormalize(result, article, entities);
});
